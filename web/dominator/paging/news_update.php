<?php 
	...
	$row = $link -> query("SELECT nc_title,nc_id FROM `news` JOIN n_class USING (nc_id) WHERE n_id = '$id'") -> fetch_row();
	$parents_id = $row[1];
		...
		<div id="wrapper">
            ...
            <script type="text/javascript">
				...
				function doupdate(){
					if(cropper.length){
		    			var img = [];
		    			for(var i in cropper) img[i] = cropper[i].getCroppedCanvas({width:img_w[i],height:img_h[i],fillColor:"rgba(0,0,0,0)"}).toDataURL('image/png',0.92);
